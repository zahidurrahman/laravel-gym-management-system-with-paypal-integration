...
//        echo"</pre>";
          ...
          $expire_date =date('Y-m-d', strtotime($payment_date. ' + 60 days'));
        ...
        if ($result->getState() == 'approved') {
            ...
            $mypay->user_id =$user_id ;
            $mypay->transaction_id =$invoice_id ;
            $mypay->currency_code =$currency_code ;
            $mypay->amount =$amount ;
            $mypay->payment_status =$result->getState();
            $mypay->payment_date =$payment_date ;
            $mypay->expire_date =$expire_date ;
            $mypay->save();
            return redirect('/home')->with('status','Payment Successfully ');

        }

        return redirect('/home')->with('status','Payment Not Successfully ');

    }

    public function show_all(){
        $get_unit = Mypay::all();
        return view('admin.manage_payment.manage_payment', compact('get_unit'));
    }
}
